...
/**
 * Created by yanyl on 2016/11/1.
 * 制作表盘
 */
public class WatchBorad  extends View {
    ...
    public WatchBorad(Context context) {
        this(context, null);
    }

    public WatchBorad(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    private float DptoPx(int value) {
        ...
        return Util.Dp2Px(getContext(), value);
    }

    //sp转px
    private float SptoPx(int value) {
        return Util.Sp2Px(getContext(), value);
    }
    ...
    private void paintScale(Canvas canvas) {
        ...
        for (int i = 0; i < 60; i++) {
            if (i % 5 == 0) { //整点
                mPaint.setStrokeWidth(Util.Dp2Px(getContext(), 1.5f));
